  ...
  return (
    <aside className="track-inspector" aria-label="Track and transition inspector">
      ...
      {nextItem && <section className={`transition-check${feedback ? ` verdict-${feedback.verdict}` : ''}`} aria-label={`Field test ${item.track.title} into ${nextItem.track.title}`}>
        <header><span><Headphones size={13} /> Try next</span><strong className="transition-verdict">{feedback?.verdict === 'compatible' ? 'Verified works' : feedback?.verdict === 'incompatible' ? 'Saved clash' : nextItem.transition.plan ? `${nextItem.transition.plan.bars}-bar test` : 'Untested'}</strong></header>
